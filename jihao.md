...
- Linux
  ...
  Linux进程下有一个小进程，查看内存命令，查看进程命令，打印一个文件中的第几行命令，查看端口命令。
...
- cnn算法
  ...
  已知输入输出求参数的个数，比如输入n*n * 3，输出为m * m * 4，求参数的个数，求一共计算的次数。

- xgb

  比gbdt的优点，如何判断是否过拟合现象，如何防止过拟合。
...
- 算法题
  ...
  已知一个字符串'5+6*(7+8)-9'，求计算的结果，并输出，可以利用两次堆栈。
